...
function getInfo(){
	var ws = Alloy.createWidget("WebService").iniciarHttpRequest({
		callback: sucesso,
		error: function(e){
			Alloy.Globals.Alerta("Erro", "Ocorreu um erro ao obter a quantidade de contratos de medições.");
			loader.stop();
			$.info.remove(loader);
			$.info.add(btnAtualizar);
		},
		url:  Alloy.Globals.MainDomain + "api/Medicao/contarMedicoes", 
		metodo: "POST", 
		timeout: 120000,
		semLoader: true
	});
...
function sucesso(ret){
	try{
		...
		var html = "<label><b><font color=\"black\">" + md.Quantidade + "</font></b><font color=\"gray\">" + 
				   "  medições de contrato precisam de sua aprovação.</font></label>";
