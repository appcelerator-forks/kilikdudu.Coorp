/**
 * @class controllers.Resumo.ResumoProcessoDePagamento
 * Mostra o resumo dos processos de pagamento. 
 * Nesta primeira versão, apresenta apenas o número de processos que carecem de aprovação deste usuário.
...
/**
 * @property {Ti.UI.Button} btnAtualizar Botão que é apresentado caso ocorra algum erro ao buscar o resumo dos processos de pagamento.
 * @private 
 */
...
/**
 * @method getInfo
 * Busca o resumo dos processos de pagamento.
 * @private
 * @alteracao 15/05/2015 185753 Projeto Carlos Eduardo Santos Alves Domingos
 * Criação.   
 */
function getInfo(){
	var ws = Alloy.createWidget("WebService").iniciarHttpRequest({
		callback: sucesso,
		error: function(e){
			Alloy.Globals.Alerta("Erro", "Ocorreu um erro ao obter a quantidade de processos de pagamento.");
			loader.stop();
			$.info.remove(loader);
			$.info.add(btnAtualizar);
		},
		url:  Alloy.Globals.MainDomain + "api/ProcessoPagamento/contarProcessos", 
		metodo: "POST", 
		timeout: 120000,
		semLoader: true
	});
...
function sucesso(ret){
	try{
		...
		var html = "<label><b><font color=\"black\">" + md.Quantidade + "</font></b><font color=\"gray\">" + "  processos de pagamento precisam de sua aprovação.</font></label>";
		...
		Alloy.Globals.onError(e.message, "sucesso", "app/controllers/Resumo/ResumoProcessoDePagamento.js");
	}
}	

/**
 * @event vai 
 * Leva o usuário até a página dos processos de pagamentos.
 * @alteracao 15/05/2015 185753 Projeto Carlos Eduardo Santos Alves Domingos
 * Criação. 
 */
function vai(e){
