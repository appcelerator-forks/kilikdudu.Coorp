 ...
 * Criação.
 */
function getEmpresasSimulacao(){
	try{
		var ws = Alloy.createWidget("WebService").iniciarHttpRequest({
			callback: sucesso,
			error: function(e){Alloy.Globals.Alerta("Erro", "Ocorreu um erro ao obter as empresas da simulação, tente novamente mais tarde.");},
			url:  Alloy.Globals.MainDomain + "api/Cotacao/getEmpresasSimulacao", 
			metodo: "POST", 
			timeout: 120000,
			semLoader: true,
			colecao: $.empresas
...
function formatar(model){
	try{
		var md = model.toJSON();
		md.lblNomeEmpresa = "Empresa: " + md.NomeEmpresa;
		...
		md.Totais = {SubTotal: md.SubTotal, Desconto: md.Desconto, Transp: md.ValorTransp, Total: md.Total, 
			ValDifICMS: md.DifAliqICMS, TotComDifICMS: md.TotalComDifICMS};
		return md;	
	}
	catch(e){
...
function detalhar(e){
	try{
		...
		var detalhes = Alloy.createController("Cotacao/ItensEmpresa", {dados: {NomeEmpresa: e.row.nome, CodFornecedor: e.row.fornecedor, NumeroSimulacao: dados.NumeroSimulacao, codCotacao: dados.codCotacao, seCotacaoGeral: dados.seCotacaoGeral, 
																			codEmpresa: e.row.empresa, TipoCotacao: dados.TipoCotacao}, totais: e.row.totais});
