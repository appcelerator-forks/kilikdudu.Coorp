 ...
 * @alteracao 21/01/2015 176562 Projeto Carlos Eduardo Santos Alves Domingos
 * Criação. 
 * @alteracao 31/08/2015 183484 Projeto Carlos Eduardo Santos Alves Domingos
 * Adicionado o número de casas decimais.
 */
$.paraReal = function(valor, separadorDecimal, numCasasDecimais)
{
	try{
		return "R$ " + $.paraFormatoDecimal(valor, separadorDecimal, numCasasDecimais);
	}
	catch(e){
		Alloy.Globals.onError(e.message, "paraReal", "app/widgets/Util/controllers/Format.js");
	}
...
/**
 * @method paraFormatoDecimal
 * Transforma um valor no formato padrão, para o formato decimal #.###,##.
 * @param {Number} valor Valor a ser formatado.
 * @param {String} separadorDecimal Caracter utilizado para separar as casas decimais do número. 
 * @param {Number} numCasasDecimais Número de casas decimais (Padrão 2).
 * @alteracao 31/08/2015 183484 Projeto Carlos Eduardo Santos Alves Domingos
 * Criação. 
 */
$.paraFormatoDecimal = function(valor, separadorDecimal, numCasasDecimais){
	try{
		if(!isNaN(valor)){
			var valor = valor.toFixed(numCasasDecimais || 2) + '';
